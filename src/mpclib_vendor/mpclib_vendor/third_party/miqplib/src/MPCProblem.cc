...
void MPCProblem::setup_variables() {
    ...
    if (yield_enable_) {
        ...
        // k=1..T
        for (int k = 1; k <= T; ++k) {
            qp.set_binary_var(y_occ[k]); // 항상 binary
            // eps_occ는 연속변수, bounds는 안 건다(아래에서 eps>=0 제약으로 처리)
        }
...
void MPCProblem::setup_problem() {
    ...
    if (n < 1) {
            ...
            for (int k = 1; k <= T; ++k) {
                ...
                // -y_k <= -y_lb  (enable이면 y_lb=0, disable이면 y_lb=1 -> y_k=1 고정)
                Constraint ylb(s);
    ...
    for (int k = 0; k <= T; ++k) {
        ...
        if (q >= 4) Qcost[pe + 3][pe + 3] += 2.0 * w_e3;
    }
    ...
    if (false) {
        const double w_vslack = 1e5; // 튜닝 파라미터 (예: 1e3~1e8)
        for (int k = 1; k <= T; ++k) {
            const int pv = eps_v[k].get_position();
            Qcost[pv][pv] += 2.0 * w_vslack; // 1/2 x^T Q x 구조라 2배
        }
    }
                                             ...
                                             const Matrix<double>& Bnew)
{
    ...
    // 아직 setup_problem()이 안 돌았으면, 다음 solve에서 새 A,B로 setup됨
    ...
    for (int k=0; k<=T; ++k) {
        ...
        // NOTE: 아래 update flags는 네 qp 구현에 따라 다를 수 있음.
        // 기존 코드가 (false,true)=RHS만 업데이트라면, LHS 업데이트는 (true,false) 또는 (true,true)여야 함.
...
void MPCProblem::set_yield_occ(bool enable, double s_stop, const std::vector<int>& allowGo)
{
    yield_enable_ = enable;

    // allowGo normalize to size T+1
    yield_allowGo_.assign(T+1, 1);
    if (enable) {
        // 입력 allowGo는 보통 size=T (k=1..T)로 들어오므로 매핑
        // allowGo[0] -> k=1, allowGo[T-1] -> k=T
        for (int k = 1; k <= T; ++k) {
            int idx = k-1;
            int v = 1;
            if (idx >= 0 && idx < (int)allowGo.size()) v = (allowGo[idx] > 0) ? 1 : 0;
            yield_allowGo_[k] = v;
        }
        // monotone 보장
        for (int k = 2; k <= T; ++k) yield_allowGo_[k] = std::max(yield_allowGo_[k], yield_allowGo_[k-1]);
        yield_s_stop_ = std::isfinite(s_stop) ? s_stop : 1e9;
    } else {
        // disable이면 stop 제약이 의미 없도록 크게
        yield_s_stop_ = 1e9;
        for (int k = 1; k <= T; ++k) yield_allowGo_[k] = 1;
    }

    // problem_setup 이후에는 RHS만 업데이트
    if (!problem_setup) return;

    // (1) stop RHS update
    for (int k = 1; k <= T; ++k) {
        int idx = k-1;
        if (idx < 0 || idx >= (int)c_y_stop_.size()) continue;

        c_y_stop_[idx].set_known_term(yield_s_stop_);
        qp.update_leq_constraint(c_y_stop_[idx], false, true);
    }

    // (2) allowGo RHS update
    for (int k = 1; k <= T; ++k) {
        int idx = k-1;
        if (idx < 0 || idx >= (int)c_y_allow_.size()) continue;

        c_y_allow_[idx].set_known_term((double)yield_allowGo_[k]);
        qp.update_leq_constraint(c_y_allow_[idx], false, true);
    }

    // (3) y lower bound switch (-y <= -ylb)
    const double y_lb = (yield_enable_ ? 0.0 : 1.0);
    for (int k = 1; k <= T; ++k) {
        int idx = k-1;
        if (idx < 0 || idx >= (int)c_y_ylb_.size()) continue;

        c_y_ylb_[idx].set_known_term(-y_lb);
        qp.update_leq_constraint(c_y_ylb_[idx], false, true);
    }
}
