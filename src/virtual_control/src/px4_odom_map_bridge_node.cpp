  ...
  : Node("px4_odom_map_bridge")
  {
    ...
    if (!target.has_value()) {
      throw std::invalid_argument(
              "route_name must be one of scenario1, scenario2, scenario3, scenario4");
    }
    ...
    RCLCPP_INFO(
      get_logger(),
      "PX4 odom map bridge ready: input=%s (yaw scalar z) output=%s route=%s "
      "target=(%.3f, %.3f, %.3f) target_yaw=%.6f rad",
      input_topic_.c_str(), output_topic_.c_str(), route_name_.c_str(),
