...
double QuadraticProblem::solve_problem(Vector<double> &arg) {
    ...
    // --------- H, f 준비 (row-major) ----------
    std::vector<c_float> H_base((size_t)n * n, (c_float)0);
    ...
    for (int r = 0; r < n; ++r) {
        f[r] = (c_float)q0[r];
        for (int c = 0; c < n; ++c) {
            const double q_rc = Q[(size_t)r][(size_t)c];
            const double q_cr = Q[(size_t)c][(size_t)r];
            const double q_sym = 0.5 * (q_rc + q_cr);
            H_base[(size_t)r * n + c] = (c_float)q_sym;
            max_asym = std::max(max_asym, std::fabs(q_rc - q_cr));
        }
        const double diag = (double)H_base[(size_t)r * n + r];
    ...
    if (debug) {
        fprintf(stderr,
            "[DAQP] Hessian diag[min,max]=[%g, %g], max_asym=%g\n",
    ...
    for (size_t attempt = 0; attempt < prox_schedule.size(); ++attempt) {
        H = H_base;
        const double diag_shift = diag_shift_base * std::pow(10.0, static_cast<double>(attempt));
        for (int i = 0; i < n; ++i) {
            // Tiny diagonal shift helps remove exact zero-curvature directions before DAQP factorization.
            H[(size_t)i * n + i] = (c_float)((double)H[(size_t)i * n + i] + diag_shift);
        }
        ...
        if (res.exitflag != EXIT_NONCONVEX) {
            break;
        }
