...
double TrackingControllerNode::computeThrottleNorm(double dv_mps, bool valid_cmd) const
{
  ...
  // PX4 normalized actuator output: 0.0 -> 1500 us, 0.2 -> 1600 us.
  ...
  const double throttle_max_norm =
    (mavlink_throttle_max_norm_ > 0.0) ? mavlink_throttle_max_norm_ : 0.2;
...
: Node("tracking_controller_node")
{
  ...
  this->declare_parameter<double>("goal_stop_distance_m", 0.30);
  ...
  this->declare_parameter<double>("mavlink_throttle_max_norm", 0.2);
  ...
  bool valid_cmd)
{
  ...
  msg.linear.z = valid_cmd ? 1.0 : 0.0;  // debug validity flag
  msg.angular.z = dpsi_rad;

  if (control_debug_pub_) {
    control_debug_pub_->publish(msg);
  }
  if (valid_cmd && applied_cmd_pub_) {
    applied_cmd_pub_->publish(msg);
  }

  publishVirtualControlCommand(steer_norm, valid_cmd);

  publishMavlinkCmd(dv_mps, steer_norm, valid_cmd);
}
  ...
  if (use_upper_guides_) {
    ...
    if (have_upper_guides_ && upper_guides_world_.size() >= 2) {
      const double guides_age =
        (last_upper_guides_rx_time_.nanoseconds() > 0) ? (now - last_upper_guides_rx_time_).seconds() : 0.0;
  ...
  std::vector<double>& preview_psi_rad) const
{
  ...
  // make_preview_constraints2:
  // if p_pred is empty -> linear preview from origin toward wp0_B.
  if (p_seed_body.size() < 2) {
    Eigen::Vector2d dir0(1.0, 0.0);
    const double d0 = wp0_body.norm();
    if (d0 > 1e-6) {
      dir0 = wp0_body / d0;
    } else if (wp1_body.norm() > 1e-6) {
      dir0 = wp1_body.normalized();
    }
    for (int k = 0; k <= n; ++k) {
